...
// combine add and remove btn
export function updateFavoList() {
  document.addEventListener("click", (event) => {
    const btn = event.target.closest(".add-favorite-btn, .remove-favorite-btn");
    if (!btn) return;
    ...
    if (btn.classList.contains("add-favorite-btn")) {
        //add-button
      if (btn.classList.contains("add-favorite-btn")) {
        if (!favoriteBooks.includes(bookId)) {
          favoriteBooks.push(bookId);
    ...
    else if (btn.classList.contains("remove-favorite-btn")) {
         removeFavoriteId(bookId);
